...
class HexPlane_Base(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.no_viewdep = False

    def turn_off_viewdep(self):
        self.no_viewdep = True
    ...
    def reinit_app_func_no_view(self):
        """
        Reinitialize appearance regression function.
        """
        self.init_app_func(
            self.AppMode,
            self.app_t_pe,
    ...
    def forward(
        ...
        if ray_valid.any():
            ...
            if hasattr(self, "no_viewdep") and self.no_viewdep:
                density_feature = self.density_regressor(
                    xyz_sampled[ray_valid],
                    torch.zeros_like(viewdirs[ray_valid]),
                    density_feature,
        ...
        if app_mask.any():
            ...
            if hasattr(self, "no_viewdep") and self.no_viewdep:
                valid_rgbs = self.app_regressor(
                    ...
                    torch.zeros_like(viewdirs[app_mask]),
                    app_features,
                    frame_time[app_mask],
                )
            else:
        ...
        if self.depth_loss:
            ...
            with torch.no_grad():
                depth_map = torch.sum(weight * z_vals, -1)
                depth_map = depth_map + (1.0 - acc_map) * rays_chunk[..., -1]
        return rgb_map, depth_map, alpha, z_vals, acc_map
