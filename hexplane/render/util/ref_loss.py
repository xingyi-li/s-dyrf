...
def nn_color_patch(top_color_centers, outputs, relation, vis=False):
    ...
    for _ in range(b):
        ...
        for center in range(color_info.shape[0]):
            ...
            temp_loss = torch.min(temp_loss,
                                  torch.sum(torch.nn.functional.mse_loss(temp_centorid, conv_res[0], reduce=False),
                                            dim=1))
        ...
        if not hasattr(self, "styl_feats_all"):
            ...
            with torch.no_grad():
                ...
                for layer in range(len(self.styl_feats_all[0])):
                    self.styl_feats_all[0][layer] = torch.concat(
                        [self.styl_feats_all[i][layer] for i in range(len(tmpl_stys))], dim=2)
                    self.tmpl_feats_all[0][layer] = torch.concat(
                        [self.tmpl_feats_all[i][layer] for i in range(len(tmpl_stys))], dim=2)
        ...
        if 'tcm_loss' in loss_names:
            ...
            for tmpl_sty in self.tmpl_stys:
                ...
                coarse_style_flat.append(F.interpolate(tmpl_sty.unsqueeze(0).permute(0, 3, 1, 2),
                                                       (h_sty // down_fact, w_sty // down_fact), mode='bilinear',
                                                       antialias=True, align_corners=True))
            ...
            coarse_out_flat = F.interpolate(outputs,
                                            (h_sty // down_fact, w_sty // down_fact), mode='bilinear', antialias=True,
                                            align_corners=True).view(1, 3, -1)
...
if __name__ == '__main__':
    ...
    loss = nnfm_loss_fn(outputs=fake_output, styles=fake_style, contents=fake_content,
