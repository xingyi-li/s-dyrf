...
def evaluation(
    ...
    if cfg.style.dataset_type == "dnerf":
        os.makedirs(savePath + "/mask", exist_ok=True)
    ...
    for idx in tqdm(idxs):
        ...
        #     rgb_map = rgb_map.cpu()
        #     rgb_map = torch.cat([rgb_map, torch.ones_like(rgb_map[:, 0:1])], dim=1) @ color_tf[:3, :4].t()
        #     rgb_map = rgb_map.clamp(0.0, 1.0)

        rgb_map, depth_map, acc_map = (
            rgb_map.reshape(H, W, 3).cpu(),
            depth_map.reshape(H, W).cpu(),
            acc_map.reshape(H, W).cpu(),
        )

        # if color_tf is not None:
        #     rgb_map = rgb_map * acc_map.unsqueeze(-1)

        if cfg.style.dataset_type == "dnerf":
            mask = (acc_map.numpy() * 255).astype("uint8")
        ...
        if savePath is not None:
            ...
            if cfg.style.dataset_type == "dnerf":
                imageio.imwrite(f"{savePath}/mask/{idx:03d}.png", mask)
            if depth is not None:
...
def evaluation_specific_pose(
    ...
    ################################
    # get specific pose
    specific_poses = cfg.style.tmpl_idx_test
    ################################
    near_far = test_dataset.near_far
    # val_rays, val_times = test_dataset.get_val_rays()
    val_rays, val_times = test_dataset.get_specific_pose_rays(specific_poses)
    ...
    for idx in tqdm(range(val_times.shape[0])):
        ...
        depth_maps.append(depth_map)
        if savePath is not None:
            if cfg.style.scene_name == "lego":
                imageio.imwrite(f"{savePath}/{prefix}{idx}.png", rgb_map)
                rgb_map = np.concatenate((rgb_map, depth_map), axis=1)
                imageio.imwrite(f"{savePath}/rgbd/{prefix}{idx}.png", rgb_map)
            else:
                imageio.imwrite(f"{savePath}/{prefix}{idx:03d}.png", rgb_map)
                rgb_map = np.concatenate((rgb_map, depth_map), axis=1)
                imageio.imwrite(f"{savePath}/rgbd/{prefix}{idx:03d}.png", rgb_map)
...
def evaluation_specific_time(
    ...
    sample_times = torch.FloatTensor([(0.5) * 2.0 - 1.0]) * test_dataset.time_scale
    val_times = sample_times.expand(val_times.shape[0], 1)
